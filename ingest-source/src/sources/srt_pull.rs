//! SRT pull source: connects to an external SRT listener and re-exposes on the internal port.

use anyhow::{Context, Result};
use gstreamer::prelude::*;
use log::info;

use crate::SrtPullConfig;

pub async fn run(id: String, internal_port: u16, config: SrtPullConfig) -> Result<()> {
    info!(
        "[{id}] srt_pull source: {}:{} → re-exposing on :{internal_port}",
    ...
    info!("[{id}] srt_pull pipeline playing");
